...
public class ApplicationBuilderWrapperTests
{
    ...
    public void UseMiddleware_AddsDescriptorWithCorrectType()
    {
        ...
        // Act
        Wrapper.UseMiddleware<TestMiddleware>();
    ...
    [Fact]
    public void Build_CallsInnerBuilderBuild()
    {
        // Arrange
        var mockInnerBuilder = new Mock<IApplicationBuilder>();
        var mockRequestDelegate = new Mock<RequestDelegate>();
        mockInnerBuilder.Setup(b => b.Build()).Returns(mockRequestDelegate.Object);

        // Act
        var result = Wrapper.Build();

        // Assert
        result.Should().Be(mockRequestDelegate.Object);
    }

    [Fact]
    public void New_CallsInnerBuilderNew()
    {
        // Arrange

        var mockApplicationBuilder = new Mock<IApplicationBuilder>();
        mockApplicationBuilder.Setup(b => b.New()).Returns(mockApplicationBuilder.Object);

        // Act
        var result = Wrapper.New();
    ...
    [Fact]
    public void ApplicationServices_ReturnsInnerBuilderApplicationServices()
    {
        // Assert
        Wrapper.ApplicationServices.Should().Be(InnerBuilder.ApplicationServices);
    }

    [Fact]
    public void ServerFeatures_ReturnsInnerBuilderServerFeatures()
    {
        // Assert
        Wrapper.ServerFeatures.Should().BeSameAs(InnerBuilder.ServerFeatures);
    }

    [Fact]
    public void Properties_ReturnsInnerBuilderProperties()
    {
        // Assert
        Wrapper.Properties.Should().BeSameAs(InnerBuilder.Properties);
    }
}
