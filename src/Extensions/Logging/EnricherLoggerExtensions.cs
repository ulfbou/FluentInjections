...
public static class EnricherLoggerExtensions
{
    /// <summary>
    ...
    /// <returns>The logger instance.</returns>
    public static Task<ILogger<TState>> WithEnricherAsync<TState>(this ILogger<TState> logger, Func<LogEntry, Task<LogEntry>> enricher)
    {
        Guard.NotNull(logger, nameof(logger));
        Guard.NotNull(enricher, nameof(enricher));
        return Task.FromResult<ILogger<TState>>(new EnrichedLogger<TState>(logger, enricher));
    }
    ...
    {
        Guard.NotNull(logger, nameof(logger));
        Guard.NotNull(additionalEnrichment, nameof(additionalEnrichment));
    ...
    {
        Guard.NotNull(logger, nameof(logger));
        Guard.NotNull(enrichers, nameof(enrichers));
    ...
    {
        Guard.NotNull(logger, nameof(logger));
        Guard.NotNull(enricherFactory, nameof(enricherFactory));
    ...
    private class EnrichedLogger<TComponentName> : ILogger<TComponentName>
    {
        private readonly ILogger<TComponentName> _innerLogger;
        ...
        public EnrichedLogger(ILogger<TComponentName> innerLogger, Func<EnricherLoggerExtensions.LogEntry, EnricherLoggerExtensions.LogEntry> synchronousEnricher)
        {
            _innerLogger = innerLogger ?? throw new ArgumentNullException(nameof(innerLogger));
            _synchronousEnricher = synchronousEnricher ?? throw new ArgumentNullException(nameof(synchronousEnricher));
            _asynchronousEnricher = null!;
        }

        public EnrichedLogger(ILogger<TComponentName> innerLogger, Func<EnricherLoggerExtensions.LogEntry, Task<EnricherLoggerExtensions.LogEntry>> asynchronousEnricher)
        {
            _innerLogger = innerLogger ?? throw new ArgumentNullException(nameof(innerLogger));
            _synchronousEnricher = null!;
        ...
        {
            var logEntry = new LogEntry(logLevel, eventId, state!, exception!, (s, e) => formatter((TState)s, e));

            if (_synchronousEnricher is not null)
            {
                logEntry = _synchronousEnricher(logEntry);
            }
            else if (_asynchronousEnricher is not null)
            {
